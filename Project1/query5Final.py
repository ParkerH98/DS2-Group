...
if __name__ == '__main__':
    ...
    nft_data = pd.read_csv("/content/nft_dataset.csv")
    nft_data.info()
    ...
    sorted_buyer_data = pd.DataFrame(final_buyer_list,
                                     columns=['Buyer', 'Different NFTs(Frequency)', 'No of Transactions(Frequency)'])
    
    sorted_buyer_data = sorted_buyer_data.merge(nft_org_data, on = "Buyer")

    sorted_buyer_data[['Buyer', 'Different NFTs(Frequency)', 'No of Transactions(Frequency)',"Token ID", "Txn Hash", "Date Time (UTC)", "NFT", "Type", "Quantity", "Price"]]
